...
public class SnakeSequence {
	...
	public static List<Pair> constructPath(int L[][], int[][] grid, Pair tail) {
		...
		while (L[i][j] != 0) {
			if (i - 1 >= 0 && L[i][j] - L[i - 1][j] == 1 && Math.abs(grid[i - 1][j] - grid[i][j]) == 1) {
				path.add(new Pair(i - 1, j));
				i--;
			}
			// note that there can be multiple paths
			else if (j - 1 >= 0 && (L[i][j] - L[i][j - 1] == 1) && Math.abs(grid[i][j - 1] - grid[i][j]) == 1) {
	...
	public static void main(String[] args) {
		int[][] grid = { { 7, 5, 2, 3, 1 }, { 3, 4, 1, 4, 4 }, { 1, 5, 6, 7, 8 }, { 3, 4, 5, 8, 9 },
				{ 3, 2, 2, 7, 6 } };
